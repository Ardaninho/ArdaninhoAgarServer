...
class Server {
    ...
    onHttpServerOpen() {
        ...
        // Done
        Logger.info("Starting the gameserver on port " + this.config.serverPort);
        Logger.info("The gamemode is " + this.mode.name);
        // Player bots (Experimental)
        if (this.config.serverBots) {
            for (var i = 0; i < this.config.serverBots; i++)
                this.bots.addBot();
            Logger.info("has been added " + this.config.serverBots + " bots");
        }
    ...
    mainLoop() {
        ...
        if (this.ticks > this.config.serverRestart) {
            ...
            if (this.config.serverBots) {
                for (var i = 0; i < this.config.serverBots; i++)
                    this.bots.addBot();
                Logger.info("was added " + this.config.serverBots + " bots");
            }
            ;
            this.commands;
            this.ticks = 0;
    ...
    loadFiles() {
        ...
        try {
            ...
            this.userList = list;
            Logger.info(this.userList.length + " uploaded user records");
        }
        catch (err) {
            Logger.error(err.stack);
        ...
        try {
            if (fs.existsSync(fileNameIpBan)) {
                ...
                Logger.info(this.ipBanList.length + " uploaded IP ban records");
            }
            else {
    ...
    startStatsServer(port) {
        ...
        this.httpServer.listen(port, function () {
            // Stats server
            Logger.info("Statistics server started on port " + port);
